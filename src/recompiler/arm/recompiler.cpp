...
#include <e32debug.h>
#include <QDebug>
#define SYMBIAN_CODECHUNK_SIZE 1024*1024*12 + (REC_MAX_OPCODES*2) + 0x4000;
static RChunk* g_code_chunk = NULL;
static RHeap* g_code_heap = NULL;
void sys_cacheflush(u32* BEG, u32* END)
{
    User::IMB_Range(BEG, END);
}
#endif
...
#define recOpcode() { \
	if (rec_phase) { \
        ...
        CALLFunc((u32)RDebug::Printf_opcode); \
...
#define recOpcode() { \
	if (rec_phase) { \
		...
		/* if (!r2_is_dirty) { */ \
			...
			/*write32(0xe8bd001f);*/ \
		/* } */ \
/*write32(0xe92d001f);MOV32ItoR(HOST_r0,(u32)"CHK");MOV32ItoR(HOST_r1,(u32)&psxM[0]);MOV32ItoR(HOST_r2,0x200000);CALLFunc((u32)RDebug::Printfsum);write32(0xe8bd001f);*/ \
/*if (pc>=0x80083000 && pc<=0x80083100) { write32(0xe92d001f);CALLFunc((u32)copia);write32(0xe8bd001f); iUpdateRegs(0); CALLFunc((u32)RDebug::Printfpsxregs); CALLFunc((u32)restaura); }*/ \
		MOV32ItoR(HOST_r3,block); \
		MOV32ItoR(HOST_r2,(((pc - pcold) / 4) * BIAS)); \
		MOV32ItoR(HOST_r1,psxRegs.code); \
		MOV32ItoR(HOST_r0,pc); \
...
static void test_r2_iocycles(unsigned r2) {
	if (r2!=psxRegs.io_cycle_counter) {
        RDebug::Printf("R2=%i != io_cycles=%i\n",r2,psxRegs.io_cycle_counter);
	}
}
#endif
...
#endif
#ifdef DEBUG_CPU
            RDebug::Printf("\t\t\t\tMapConst r%i=%p",reg,_const);
#endif
	UnmapGteReg(reg);
	if (IsMapped(reg)) {
#ifdef DEBUG_CPU
        RDebug::Printf(" (!R%i)",iRegs[reg].reg);
#endif
...
static void iClearRegs() {
    ...
    HWRegs[9].state = ST_CONST; //ST_UNK; SYMBIAN
...
#endif
#ifdef DEBUG_CPU
    RDebug::Printf("\t\t\tReadWriteReg r%i\n",reg);
#endif
	UnmapGteReg(reg);
	if (iRegs[reg].state!=ST_MAPPED)
	{
		...
		if ((rec_cache>>i)&1) {
			...
			switch(regs[i].state) {
...
#ifdef DEBUG_CPU
                                RDebug::Printf("MOV R%i, R%i and ",j,reg);
#endif
								MOV32RtoR(j,reg);
								break;
							}
				...
				case ST_MAPPED:
					if (regs[i].reg==reg) {
#ifdef DEBUG_CPU
                        RDebug::Printf(" MAPPED OK");
#endif
						break;
					}
					if (hw[reg].state==ST_MAPPED) {
#ifdef DEBUG_CPU
                        RDebug::Printf(" MAPPED MAPPED: ");
#endif
						for(j=0;j<16;j++) {
							if (hw[j].state==ST_UNK) {
								u32 nreg=hw[reg].reg;
...
#ifdef DEBUG_CPU
                    RDebug::Printf("\t\tRecompiled Cache=%.8X\n",rec_cache);
#endif
...
static int recInit() {
    int i;
    TInt minsize = RECMEM_SIZE + (REC_MAX_OPCODES*2) + 0x4000;
    TInt maxsize = minsize + 3 * 4096;
    ...
    recMem = (char*)g_code_heap->Alloc(RECMEM_SIZE + (REC_MAX_OPCODES*2) + 0x4000);
...
static void recFunctions() {
...
#ifdef REC_USE_RETURN_FUNCS
#ifdef DEBUG_CPU
    RDebug::Printf("-- FUNCTION TestBranchIfOK --");
#endif
	recTestBranchIfOK(0);
	func_Return_ptr=(unsigned)armPtr;
#ifdef DEBUG_CPU
    RDebug::Printf("-- FUNCTION Return --");
#endif
	recReturn(0); gen_align4();
#ifdef REC_USE_FAST_BLOCK
	func_TestBranchBlock_ptr=(unsigned)armPtr;
#ifdef DEBUG_CPU
    RDebug::Printf("-- FUNCTION TestBranchBlock --");
#endif
...
#ifdef DEBUG_CPU
    RDebug::Printf("-- FUNCTION GTE_MFC2 --");
#endif
...
#ifdef DEBUG_CPU
    RDebug::Printf("-- FUNCTION GTE_MTC2_15 --");
#endif
...
#ifdef DEBUG_CPU
    RDebug::Printf("-- FUNCTION GTE_MTC2_28 --");
#endif
...
#ifdef DEBUG_CPU
    RDebug::Printf("-- FUNCTION GTE_updateMACs0_flag --");
#endif
...
#ifdef DEBUG_CPU
    RDebug::Printf("-- FUNCTION GTE_updateMACs1_flag --");
#endif
#if defined(REC_USE_GTE_FUNCS) && defined(REC_USE_GTE_DELAY_CALC)
	MOV32ItoM_regs((u32)&psxRegs.CP2C.r[31],0);
#endif
...
#ifdef DEBUG_CPU
    RDebug::Printf("-- FUNCTION GTE_updateMAC3_flag --");
#endif
#if defined(REC_USE_GTE_FUNCS) && defined(REC_USE_GTE_DELAY_CALC)
	MOV32ItoM_regs((u32)&psxRegs.CP2C.r[31],0);
#endif
	...
	if (armPtr>(u32 *)&recMem[RECMEM_SIZE + (REC_MAX_OPCODES*2) + 0x4000]){
        RDebug::Printf("recFunctions memory overflow");
		pcsx4all_exit();
	}
	UpdateImmediate(1);
...
#ifdef DEBUG_CPU
            RDebug::Printf("\tCached R%i ",rec_total_opcodes);
#endif
...
// CHUI: Tenemos que controlar que nos pasamos de los 4096 max para el offset de los inmediatos
		if (immCount && !branch)
			if (((unsigned)armPtr)-immPtr[0] > 3072)
			{
				...
				if (rec_total_opcodes>REC_MAX_OPCODES_LIMIT) // Si falta poco para terminar salimos
					break;
#ifdef DEBUG_CPU
                RDebug::Printf("\t\tForzado UpdateImmediate");
#endif
    ...
    RDebug::Printf("<-%i Opcodes, size�o blocks=%i bytes (%.2f for opcode and %.2f average)",rec_total_opcodes,((unsigned)armPtr)-((unsigned)armPtr_old),numero,media);
    RDebug::Printf("  %i Immediates, Used %i bytes (%.2f for immediate and %.2f average)",immediates,immsize,numero2,media2);
    RDebug::Printf("  %.2f%% records used (%.2f%% average)",oks,media3);
#endif
}
